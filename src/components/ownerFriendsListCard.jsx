...
const Friendslistcard =  ( props ) => {
        ...
        <div className='flex py-2 md:py-3 px-1 md:px-3 items-center border' 
           ...
           {check?navigate(``):navigate(`/profile/${friend._id}`)}
            ...
            {loading?
                    <div class="animate-pulse rounded-full bg-slate-200 h-7 md:h-10 w-7 md:w-10 my-auto">
                    </div>
                    :
                <img src={friend.picturePath} className='rounded-full object-cover h-7 md:h-10 w-7 md:w-10' alt='user' />}
            </div>
            ...
            {loading?
                    ...
                    :
                <div className="w-12 md:w-auto font-bold text-[0.7rem] capitalize md:text-sm text-gray-800">{`${friend.firstName} ${friend.lastName}`}</div>}
            {loading?
                <div class="animate-pulse mb-2 px-2 w-3/4  my-auto">
                    <div class="h-2 bg-slate-200 rounded"></div>   
                </div>
                :
                <div className='text-muted text-gray-500 text-[0.7rem] md:text-xs'>{friend.occupation}</div>}
            </div>
            {friend._id != user._id?
