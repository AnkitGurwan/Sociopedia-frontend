          ...
          <div className='flex items-center w-12 pl-2 text-sm cursor-pointer' onClick={likeHandler}>
            {liked ? (
              <div className='flex items-center'>
                <span onClick={() => { setLiked(false); setLikeCount(likeCount - 1); }} class='material-symbols-outlined p-1 text-lg text-red-700'>
                  <svg xmlns='http://www.w3.org/2000/svg' fill='red' viewBox='0 0 24 24' stroke-width='1' stroke='red' class='w-6 h-6 border-none'>
                    <path stroke-linecap='round' stroke-linejoin='round' d='M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z' />
                  </svg>
                </span>
                <span className=' text-pink-600 fill-pink-600'>{likeCount}</span>
              </div>
            ) : (
              <div className='flex items-center'>
                <span onClick={() => { setLiked(true); setLikeCount(likeCount + 1); }} class={'material-symbols-outlined p-1 text-lg text-gray-300 ' + (mode === 'light' ? 'text-gray-800' : 'text-gray-300')}>
                  <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' stroke-width='1' stroke='currentColor' class='w-6 h-6'>
                    <path stroke-linecap='round' stroke-linejoin='round' d='M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z' />
                  </svg>
                </span>
                <span className={'text-gray-300 ' + (mode === 'light' ? 'text-gray-800' : 'text-gray-300')}>{likeCount}</span>
              </div>
            )}
          </div>
          <div className='flex items-center p-2 text-sm'>
            <span class={'material-symbols-outlined p-1 text-lg ' + (mode === 'light' ? 'text-gray-800' : 'text-gray-300')}>chat_bubble</span>
            <span className={' ' + (mode === 'light' ? 'text-gray-800' : 'text-gray-300')}>{comments.length}</span>
          </div>
        </div>
      </div>
    </div>
