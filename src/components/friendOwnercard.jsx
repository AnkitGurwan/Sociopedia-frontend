...
const Ownercard = () => {
    ...
    return (
        <div className="rounded-lg shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] px-2 md:px-4 py-1 bg-gray-100 border">
            <div className='h-16 md:h-20 flex py-2 items-center w-full'>
                ...
                {loading?
                    ...
                    :
                    <img src={user.picturePath} className='rounded-full object-cover h-7 md:h-10 w-7 md:w-10' alt='user' />}
                </div>
                <div className='w-28 md:w-52 pl-2 md:pl-6 pr-1'>
                {loading?
                    <div class="animate-pulse mb-2 px-2 w-3/4  my-auto">
                        <div class="h-2 bg-slate-200 rounded"></div>   
                    </div>
                    :
                    <div className="font-bold text-[0.7rem] md:text-lg ml-1 mb-1 capitalize">{`${user.firstName} ${user.lastName}`}
                    </div>}
                    {loading?
                    <div class="animate-pulse mb-2 px-2 w-3/4  my-auto">
                        <div class="h-2 bg-slate-200 rounded"></div>   
                    ...
                    :
                        <div className='text-muted text-gray-500 text-[0.7rem] md:text-xs bg-gray-200 w-fit md:w-1/2 px-2 rounded-sm'>Following : {user.friends?user.friends.length:0}</div>}
                </div>
            </div>
            <hr/>
            ...
            <div className='h-24 md:h-20 py-3 px-3'>
                <div className='flex items-center h-1/2 text-xs justify-between pb-2 md:pb-0'>
                    <div className='text-[0.7rem] md:text-sm text-gray-500'>Who's viewed your profile.</div>
                    {loading?
                ...
                <div className='flex items-center h-1/2 text-xs justify-between '>
                    <div className='text-[0.7rem] md:text-sm text-gray-500 '>Impressions of your post</div>
                    {loading?
                    <div class="animate-pulse mb-2 px-2 w-10  my-auto">
                        <div class="h-2 bg-slate-200 rounded"></div>   
                    </div>
